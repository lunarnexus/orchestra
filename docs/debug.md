...
- **Lifecycle log**: `logs/<run-id>.jsonl`; lean Orchestra-owned events such as
  supervisor spawn/start/failure, worker start/exit, artifact writes, and state
  updates.
...
- **Return artifact**: `state/return-artifacts/<run-id>.md`; worker stdout/stderr
  and returned content when the worker starts and produces output.
- **Harness transcript**: optional. Pi workers use a deterministic worker session
...
The worker hard timeout starts only after the detached supervisor starts the
worker. If the supervisor dies before moving a run from `queued` to `running`,
there may be no worker process to time out. Orchestra opportunistically
