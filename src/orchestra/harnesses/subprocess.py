"""Generic one-shot subprocess harness implementation."""
...
WorkerSessionMode = Literal["none", "pi"]
...
class SubprocessHarness:
    ...
    def start(self, request: WorkerRequest, role: RoleConfig) -> WorkerProcess:
        ...
        command = _apply_worker_session(
            self.build_command(role, prompt),
            worker_session_id,
            mode=self.worker_session_mode,
        )
        ...
        return WorkerProcess(
            ...
            command=command,
            prompt=prompt,
            worker_session_id=(worker_session_id if self.worker_session_mode != "none" else None),
        )
...
def _apply_worker_session(
    ...
    *,
    ...
    if mode == "pi":
        return _with_pi_worker_session(command, worker_session_id)
    return command


def _with_pi_worker_session(command: list[str], worker_session_id: str | None) -> list[str]:
