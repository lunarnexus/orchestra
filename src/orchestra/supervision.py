...
class AutomaticDispatchChain:
    trigger_role: str
    trigger_status: str
    next_role: str
    trigger_reason: str
    sequence_index: int
    task_label_template: str
    return_format: str
    dispatch_failure_event: str
    dispatch_failure_label: str
    suppress_child_hints: bool = False

    def matches(self, run: RunRecord) -> bool:
        return run.role == self.trigger_role and run.status == self.trigger_status

    def task_label_for(self, run: RunRecord) -> str:
        return self.task_label_template.format(run_id=run.run_id, role=run.role)

    def return_format_for(self) -> str:
        return self.return_format

    def dispatch_failure_note(self, error_type: str | None, error: str | None) -> str:
        if error_type and error:
            return f"{self.dispatch_failure_label} dispatch failed: {error_type}: {error}"
        if error:
            return f"{self.dispatch_failure_label} dispatch failed: {error}"
        return f"{self.dispatch_failure_label} dispatch failed"

    def auto_return_key(self) -> str:
        return self.trigger_reason

    def auto_return_event(self) -> str:
        return self.dispatch_failure_event

    def child_hints_suppressed(self) -> bool:
        return self.suppress_child_hints

    def should_hold_parent_report(self, run: RunRecord) -> bool:
        return (
            self.suppress_child_hints
            and run.role == self.trigger_role
            and run.status == self.trigger_status
        )

    def duplicate_triggered_child_match(self, run: RunRecord, other: RunRecord) -> bool:
        return (
            run.run_id != other.run_id
            and other.triggered_by_run_id == run.run_id
            and other.trigger_reason == self.trigger_reason
        )
    ...
    def build_handoff(
        ...
        return build_auto_verifier_assignment(builder_run, builder_request)


AUTO_VERIFY_CHAIN = AutomaticDispatchChain(
    trigger_role="builder",
    trigger_status=STATUS_DONE,
    next_role="verifier",
    trigger_reason="auto_verify",
    sequence_index=1,
    task_label_template="auto verify {run_id}",
    return_format=(
    ...
    dispatch_failure_event="auto_verify.dispatch_failed",
    dispatch_failure_label="auto-verify",
    suppress_child_hints=True,
)

AUTOMATIC_DISPATCH_CHAINS = {AUTO_VERIFY_CHAIN.trigger_reason: AUTO_VERIFY_CHAIN}
...
def _auto_dispatch_chain_for_run(builder_run: RunRecord) -> AutomaticDispatchChain | None:
    for chain in AUTOMATIC_DISPATCH_CHAINS.values():
        if chain.matches(builder_run):
            return chain
    return None
...
def run_supervisor(context: AppContext, *, run_id: str, request_file: str | Path) -> RunRecord:
    ...
    selected_role = _select_role(
        context.catalog,
        record.role,
        allow_auto_only=record.trigger_reason == AUTO_VERIFY_CHAIN.trigger_reason,
    )
